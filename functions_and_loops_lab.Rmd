...
We can turn that basic code into a basic function.
```{r}
...
First let's make a test vector with some `NA`s.  (Also, let's learn about indexing to access specific elements in a vector).
```{r}
...
We can use this function anywhere we might use the regular `base::mean()`.  All it needs is a vector, which includes a column from a dataframe!
...
## Using our function inside another function
...
``` {r}
### explore the distribution of die rolls
...
OK cool, now let's make a loop to calculate a bunch of simulations.  Here let's simulate rolling a die 20 times, and calculate the mean and standard deviation.  Then we will repeat that experiment a bunch of times, i.e. iterate for many simulations.

``` {r}
### set up for a lot of simulations
sims <- 10000

### initialize empty results vectors that I will populate in the loop
mean_results <- vector('numeric', length = sims)
sd_results <- vector('numeric', length = sims)

for(i in 1:sims) {
  test_vec <- sample(x = 1:6, size = 20, replace = TRUE)
  sim_mean <- calc_mean2(test_vec)
  mean_results[i] <- sim_mean
  sd_results[i] <- calc_sd(test_vec)
}
```
...
results_df <- data.frame(sim = 1:sims,
                         mean = mean_results,
                         sd   = sd_results) %>%
  pivot_longer(names_to = 'param', values_to = 'value', -sim)

ggplot(results_df, aes(x = value, fill = param)) +
